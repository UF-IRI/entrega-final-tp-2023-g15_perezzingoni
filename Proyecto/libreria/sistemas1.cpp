...
eAgrClientes AgregarCliente(sClientes *Clientela, sCliente Cliente){
    ...
    sCliente nulo;
    eSrchClientes client;
    client= BuscarCliente(Clientela,Cliente.idCliente,nulo);
    if(client!=ErrSrchNoExite)
        return eAgrClientes::ErrAgrExiste;

    if(Clientela->CantClientes==Clientela->CantMaxima)
        resizeClientes(Clientela,Clientela->CantMaxima,Clientela->CantMaxima+5);
...
void resizeClientes(sClientes * Clientela, uint tam, uint nuevoTam) {
    if(Clientela->misClientes!=nullptr){
